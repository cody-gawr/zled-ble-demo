...
class RestManager {
    ...
    func makeRequest<T: Decodable>(toURL url: URL, withHttpMethod httpMethod: HttpMethod, completion: @escaping (_ result: Results<T>) -> Void) {
...
extension RestManager {
    ...
    struct RestEntity {
        ...
        mutating func setValues(values: [String: String]) {
            self.values = values
        }
    ...
    struct Results<T: Decodable> {
        ...
        var data: T? {
            ...
            guard let data = try? decorder.decode(T.self, from: rawData) else { return nil }
            return data
        }
...
extension RestManager.CustomError: LocalizedError {
    public var localizedDescription: String {
        switch self {
        case .failedToCreateRequest:
            return NSLocalizedString(Constant.Strings.Errors.unableToCreateURLRequest, comment: "")
        }
    }
}
